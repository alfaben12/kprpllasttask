...
class Warnas extends CI_Controller{
	
	function __construct(){
		parent :: __construct();
		/* Load model */
		$this->load->model('warna');
	}

	function index(){
		/* function ambil semua data */
		$list = $this->warna->ambil_semua_data();
	...
	function prosess_tambah(){
		/* atur form validasi */
		$this->form_validation->set_rules('txt_warna', 'Warna', 'trim|required|xss_clean');
	...
	function edit(){
		...
		/* membuat value */
		$this->warna->setID($arr['id']);

		/* fungsi ambil data */
		$result = $this->warna->ambil_data();
